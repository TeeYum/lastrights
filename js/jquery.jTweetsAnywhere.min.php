...
{
	'$$monthNames': [ "<?php _e('Jan', 'cmsmasters'); ?>", "<?php _e('Feb', 'cmsmasters'); ?>", "<?php _e('Mar', 'cmsmasters'); ?>", "<?php _e('Apr', 'cmsmasters'); ?>", "<?php _e('May', 'cmsmasters'); ?>", "<?php _e('Jun', 'cmsmasters'); ?>", "<?php _e('Jul', 'cmsmasters'); ?>", "<?php _e('Aug', 'cmsmasters'); ?>", "<?php _e('Sep', 'cmsmasters'); ?>", "<?php _e('Oct', 'cmsmasters'); ?>", "<?php _e('Nov', 'cmsmasters'); ?>", "<?php _e('Dec', 'cmsmasters'); ?>" ]
});
...
(function($)
{
	...
	{
		return '<span class="jta-tweet-list-controls-button jta-tweet-list-controls-button-more">' + options._resourceBundle._("<?php _e('More', 'cmsmasters'); ?>") + '</span>';
	};
	defaultTweetFeedControlsPrevBtnDecorator = function(options)
	{
		return '<span class="jta-tweet-list-controls-button jta-tweet-list-controls-button-prev">' + options._resourceBundle._("<?php _e('Prev', 'cmsmasters'); ?>") + '</span>';
	};
	defaultTweetFeedControlsNextBtnDecorator = function(options)
	{
		return '<span class="jta-tweet-list-controls-button jta-tweet-list-controls-button-next">' + options._resourceBundle._("<?php _e('Next', 'cmsmasters'); ?>") + '</span>';
	};
	...
	{
		var content = options._resourceBundle.__("%count% <?php _e('new tweet', 'cmsmasters'); ?>", "%count% <?php _e('new tweets', 'cmsmasters'); ?>", count, { '%count%' : count });
	...
	defaultTweetTextDecorator = function(tweet, options)
	{
		...
		return '<span class="jta-tweet-text entry-content tweet_content">' + tweetText + '</span>';
	};
	...
	defaultTweetTwitterBirdDecorator = function(tweet, options)
	{
		...
		var linkTitle = screenName + ' ' + options._resourceBundle._("<?php _e('on Twitter', 'cmsmasters'); ?>");
	...
	defaultTweetTimestampFormatter = function(timeStamp, options)
	{
		...
		if (diff < 60)
		{
			tweetTimestamp += options._resourceBundle.__("%secs% <?php _e('second ago', 'cmsmasters'); ?>", "%secs% <?php _e('seconds ago', 'cmsmasters'); ?>", diff, { '%secs%': diff });
		}
		else if (diff < 3600)
		{
			var t = parseInt((diff + 30) / 60);
			tweetTimestamp += options._resourceBundle.__("%mins% <?php _e('minute ago', 'cmsmasters'); ?>", "%mins% <?php _e('minutes ago', 'cmsmasters'); ?>", t, { '%mins%': t });
		}
		else if (diff < 86400)
		{
			var t = parseInt((diff + 1800) / 3600);
			tweetTimestamp += options._resourceBundle.__("%hours% <?php _e('hour ago', 'cmsmasters'); ?>", "%hours% <?php _e('hours ago', 'cmsmasters'); ?>", t, { '%hours%': t });
		}
		else
		{
			var d = new Date(timeStamp);
			...
			tweetTimestamp += ' (' + options._resourceBundle.__("%days% <?php _e('day ago', 'cmsmasters'); ?>", "%days% <?php _e('days ago', 'cmsmasters'); ?>", t, { '%days%': t }) + ')';
		}
	...
	defaultTweetSourceDecorator = function(tweet, options)
	{
		...
		var html =
			...
			' ' + options._resourceBundle._("<?php _e('via', 'cmsmasters'); ?>") + ' ' +
	...
	defaultTweetGeoLocationDecorator = function(tweet, options)
	{
		...
		if (q)
		{
			var location = options._resourceBundle._("<?php _e('here', 'cmsmasters'); ?>");
			...
			html =
				...
				' ' + options._resourceBundle._("<?php _e('from', 'cmsmasters'); ?>") + ' ' +
	...
	defaultTweetInReplyToDecorator = function(tweet, options)
	{
		...
		if (tw.in_reply_to_status_id && tw.in_reply_to_screen_name)
		{
			...
			var linkText = options._resourceBundle._("<?php _e('in reply to', 'cmsmasters'); ?>") + ' ' + tw.in_reply_to_screen_name;
	...
	defaultTweetRetweeterDecorator = function(tweet, options)
	{
		...
		if (tweet.retweeted_status)
		{
			...
			var rtcount = options._resourceBundle.__(" <?php _e('and', 'cmsmasters'); ?> %rtc% <?php _e('other', 'cmsmasters'); ?>", " <?php _e('and', 'cmsmasters'); ?> %rtc% <?php _e('others', 'cmsmasters'); ?>", rtc, { '%rtc%': rtc });

			html =
				'<br/>' +
				'<span class="jta-tweet-retweeter">' +
				options._resourceBundle._("<?php _e('Retweeted by', 'cmsmasters'); ?>") + ' ' + link +
				(rtc > 0 ? rtcount : '') +
				'</span>';
		}
	...
	defaultTweetActionReplyDecorator = function(tweet, options)
	{
		...
		var actionLabel = options._resourceBundle._("<?php _e('Reply', 'cmsmasters'); ?>");
	...
	defaultTweetActionRetweetDecorator = function(tweet, options)
	{
		...
		var actionLabel = options._resourceBundle._("<?php _e('Retweet', 'cmsmasters'); ?>");
	...
	defaultTweetActionFavoriteDecorator = function(tweet, options)
	{
		...
		var actionLabel = options._resourceBundle._("<?php _e('Favorite', 'cmsmasters'); ?>");
	...
	defaultLoginInfoContentDecorator = function(options, T)
	{
		...
		if (T.isConnected())
		{
			...
			html =
				...
				options._resourceBundle._("<?php _e('Sign out', 'cmsmasters'); ?>") +
				'</div>' +
				'</div>' +
	...
	defaultLoadingDecorator = function(options)
	{
		...
		return '<li class="jta-loading">' + options._resourceBundle._("<?php _e('Loading', 'cmsmasters'); ?>") + '...</li>';
	};
	defaultErrorDecorator = function(errorText, options)
	{
		// the default error decorator shows the error message
		return '<li class="jta-error">' + options._resourceBundle._("<?php _e('ERROR', 'cmsmasters'); ?>") + ': ' + errorText + '</li>';
	};
	defaultNoDataDecorator = function(options)
	{
		// the default no-data decorator simply says: No more data
		return '<li class="jta-nodata">' + options._resourceBundle._("<?php _e('No more data', 'cmsmasters'); ?>") + '</li>';
	};
	...
	setupOptions = function(options)
	{
		...
		options._tweetBoxConfig.label = options._resourceBundle._("<?php _e("What's happening?", 'cmsmasters'); ?>");
