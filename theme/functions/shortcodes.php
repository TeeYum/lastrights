...
function cmsmasters_accordion($atts, $content = null) {
    ...
    $content = str_replace('<a class="tog" href="#">', '<a class="tog button_large" href="#">', do_shortcode($content));
...
function cmsmasters_tour($atts, $content = null) {
    ...
    foreach ($atts as $tour) {
        $out .= '<li><a href="#"><span>' . $tour . '</span></a></li>';
    }
...
function cmsmasters_divider_top() {
    return '<div class="divider">' . 
        '<a class="fr" href="#">' . __('Top', 'cmsmasters') . ' &uarr;</a>' . 
        '<div class="cl"></div>' . 
    '</div>';
}
...
function posttype_shortcode($atts, $content = null) {
    ...
    if ($posttype_query->have_posts()) :
        while ($posttype_query->have_posts()) : $posttype_query->the_post();
			$out .= '<article class="post hentry' . (($page_layout == 'nobg') ? ' one_fourth' : ' one_third') . '">';
			...
			if ($show_images == 'true') {
				if ($type == 'slider' || $type == 'album' || $type == 'gallery') {
					if (has_post_thumbnail()) {
						$out .= cmsms_thumb(get_the_ID(), 440, 250, true, false, true, false, false, false);
							...
							if (!isset($counter) && $counter = true) {
								$out .= '<figure>' . 
									'<a class="preloader" href="' . get_permalink() . '"' . ' title="' . cmsms_title(get_the_ID(), false) . '">' . 
										'<img src="' . get_template_directory_uri() . '/theme/classes/timthumb.php?src=' . get_image_path_attachments($attachment->guid) . '&amp;w=440&amp;h=250&amp;zc=1' . '" alt="' . cmsms_title(get_the_ID(), false) . '" class="fullwidth" />' . 
									'</a>' . 
								'</figure>';
							}
						}
						...
						$out .= '<figure>' . 
							'<a class="preloader" href="' . get_permalink() . '"' . ' title="' . cmsms_title(get_the_ID(), false) . '">' . 
								'<img src="' . get_template_directory_uri() . '/theme/classes/timthumb.php?src=' . get_template_directory_uri() . '/images/PT-gallery.jpg&amp;w=440&amp;h=250&amp;zc=1' . '" alt="' . cmsms_title(get_the_ID(), false) . '" class="fullwidth" />' . 
							'</a>' . 
						'</figure>';
					}
				} else {
					if (has_post_thumbnail() && $type != 'video') {
						$out .= cmsms_thumb(get_the_ID(), 440, 250, true, false, true, false, false, false);
					} else {
						$out .= '<figure>' . 
							'<a class="preloader" href="' . get_permalink() . '"' . ' title="' . cmsms_title(get_the_ID(), false) . '">' . 
								'<img src="' . get_template_directory_uri() . '/theme/classes/timthumb.php?src=' . get_template_directory_uri() . '/images/PT-' . (($type == 'image' || $type == '') ? 'placeholder' : $type) . '.jpg&amp;w=440&amp;h=250&amp;zc=1' . '" alt="' . cmsms_title(get_the_ID(), false) . '" class="fullwidth" />' . 
							'</a>' . 
						'</figure>';
					}
				}
			...
			if ($type != 'aside' && $type != 'link' && $type != 'quote') {
				...
				$out .= '<header class="entry-header">' . 
					'<h6 class="entry-title">' . 
						'<a href="' . $post_link_link . '" title="' . $post_link_text . '">' . $post_link_text . '</a>' . 
					'</h6>' . 
				'</header>';
			} elseif ($type == 'aside') {
				$out .= '<header class="entry-header">';
				...
				if (has_excerpt()) {
					$out .= '<h5 class="entry-title">' . get_the_excerpt() . '</h5>';
				} else {
					global $more;
					
					$more = 0;
					
					$out .= '<h5 class="entry-title">' . get_the_content('') . '</h5>';
				}
				...
				if (has_excerpt()) {
					...
					$out .= '<h5 class="entry-title">' . __('Enter post excerpt', 'cmsmasters') . '</h5>';
				}
...
function cmsmasters_slider($atts, $content = null) {
    ...
    foreach ($sliderOptions['slider']['slides'] as $slide) {
        ...
        $out .= '>' . 
			'<img src="' . $slide['slide_link'] . '" alt="' . $slide['slide_title'] . '" />';
		
        if ($show_caption == 'true') {
            $out .= '<div class="slideCaption' . (($slide_caption_pos == 'left') ? ' left' : ' right') . '">' . 
            ...
            if ($caption_subtitle != '') {
                $out .= '<h5>' . $caption_subtitle . '</h5>';
            }
			
            if ($caption_text != '') {
                $out .= '<p>' . $caption_text . '</p>';
            }
