 ...
 * @since Oakland 1.1
 * 
 * Divider Shortcodes & Quick Tags Register
 * Created by CMSMasters
 * 
 */


if (!class_exists('CMSMastersDivider')) {
	class CMSMastersDivider {
		var $buttonName;
		var $buttonTitle;
		var $buttonArray;
		
		function __construct() {
			$this->buttonName = 'divider';
			$this->buttonTitle = __('Dividers', 'cmsmasters');
			$this->buttonArray = array(
                0 => array(__('Divider Top', 'cmsmasters'), 'divider_top'), 
                1 => array(__('Divider', 'cmsmasters'), 'divider'), 
                2 => array(__('Clear', 'cmsmasters'), 'clear') 
            );
		}
...
if (!isset($cmsmasters_shortcode_divider)) {
	...
	add_action('admin_head', array($cmsmasters_shortcode_divider, 'addDropdown'));
    add_action('admin_enqueue_scripts', array($cmsmasters_shortcode_divider, 'registerQtagPluginButton'));
}
