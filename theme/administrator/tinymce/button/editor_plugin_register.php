 ...
 * @since Oakland 1.1
 * 
 * Button Shortcodes & Quick Tags Register
 * Created by CMSMasters
 * 
 */


if (!class_exists('CMSMastersButton')) {
	class CMSMastersButton {
		var $buttonName;
        var $buttonTitle;
		
		function __construct() {
			$this->buttonName = 'button';
			$this->buttonTitle = __('Button', 'cmsmasters');
		}
...
if (!isset($cmsmasters_shortcode_button)) {
	...
	add_action('admin_head', array($cmsmasters_shortcode_button, 'addButton'));
    add_action('admin_enqueue_scripts', array($cmsmasters_shortcode_button, 'registerQtagPluginButton'));
}
