 ...
 * @since Oakland 1.1
 * 
 * Google Map Shortcode & Quick Tag Register
 * Created by CMSMasters
 * 
 */


if (!class_exists('CMSMastersMap')) {
	class CMSMastersMap {
		var $buttonName;
        var $buttonTitle;
		...
		function __construct() {
			$this->buttonName = 'map';
			$this->buttonTitle = __('Google Map', 'cmsmasters');
		}
...
if (!isset($cmsmasters_shortcode_map)) {
	...
	add_action('admin_head', array($cmsmasters_shortcode_map, 'addButton'));
    add_action('admin_enqueue_scripts', array($cmsmasters_shortcode_map, 'registerQtagPluginButton'));
}
